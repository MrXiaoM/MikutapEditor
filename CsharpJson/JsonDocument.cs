...
namespace CsharpJson
{
    ...
    public class JsonDocument
    {
        ...
        public static JsonDocument FromString(string jsonstr, JsonEncoding encod = JsonEncoding.Default)
        {
            ...
            string s = jsonstr.TrimStart();
            while (s.StartsWith(" ") || s.StartsWith("\n") || s.StartsWith("\r")) s = s.Substring(1);
            int index = 0;
            if (s[0] == '{')
            {
                doc.Object = GetObject(ref index, s).ToObject();
            }
            else if (s[0] == '[')
            {
                doc.Array = GetArray(ref index, s).ToArray();
            }
            return doc;
        }
            ...
            foreach (KeyValuePair<string, JsonValue> item in items)
            {
                switch (item.Value.Valuetype)
                {
                    case JsonType.NULL:
                        ...
                        break;
                    case JsonType.BOOL:
                        str.AppendFormat("\"{0}\":{1},", item.Key, item.Value.ToBool().ToString().ToLower());
                        break;
                    ...
                    case JsonType.OBJECT:
                        str.AppendFormat("\"{0}\":{1},", item.Key, ToJsonString(item.Value.ToObject()));
                        break;
                }
            }
            ...
            foreach (JsonValue item in arrylist)
            {
                switch (item.Valuetype)
                {
                    case JsonType.NULL:
                        ...
                        break;
                    case JsonType.BOOL:
                        str.AppendFormat("{0},", item.ToBool().ToString().ToLower());
                        break;
                    case JsonType.NUMBER:
                    ...
                    case JsonType.ARRAY:
                        ...
                        break;
                    case JsonType.OBJECT:
                        str.AppendFormat("{0},", ToJsonString(item.ToObject()));
                        break;
                }
            }
            str.Replace(',', ']', str.Length - 1, 1);
